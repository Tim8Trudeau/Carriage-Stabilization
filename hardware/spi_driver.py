...
# --- choose real or mock at import time (just like pwm_driver) ---
_FORCE_MOCK = os.getenv("CS_HW", "auto").lower() == "mock"
_HAVE_PIGPIO = False
_pigpio = None
...
if not _HAVE_PIGPIO:
    # Fall back to simulation mock (try test.mocks first, then local)
    try:
        from test.mocks.mock_spi import MockSPIBus as SPIBus  # type: ignore
        spi_log.warning("SPIBus using MockSPIBus (pigpio unavailable or CS_HW=mock).")
    except Exception:
        try:
            from mock_spi import MockSPIBus as SPIBus  # type: ignore
            spi_log.warning("SPIBus using MockSPIBus (pigpio unavailable or CS_HW=mock).")
        except Exception as e:
            spi_log.error("Neither pigpio nor MockSPIBus available: %s", e)
            raise

# If we have pigpio (and not forcing mock), expose the real SPIBus implementation.
if _HAVE_PIGPIO:
    class SPIBus:
        """
